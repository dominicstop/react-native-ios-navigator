 ...
 */
@objc protocol ScrollingNavigationControllerDelegate: NSObjectProtocol {
 ...
 */
@objc enum NavigationBarState: Int {
 ...
 */
@objc enum NavigationBarCollapseDirection: Int {
  case scrollUp = -1
  case scrollDown = 1
}
...
/**
 ...
 - scrollDown: scrolling down direction
 */
@objc enum NavigationBarFollowerCollapseDirection: Int {
  case scrollUp = -1
  case scrollDown = 1
}
 ...
 */
@objcMembers
class NavigationBarFollower: NSObject {
  weak var view: UIView?
  var direction = NavigationBarFollowerCollapseDirection.scrollUp
  var changeAlphaWhileCollapsing = false
  
  init(view: UIView, direction: NavigationBarFollowerCollapseDirection = .scrollUp,
 ...
 */
@objcMembers
class ScrollingNavigationController: UINavigationController, UIGestureRecognizerDelegate {
   ...
   */
  weak var scrollingNavbarDelegate: ScrollingNavigationControllerDelegate?
  ...
  /**
   ...
   Defaults to `true`
   */
  var shouldUpdateContentInset = true
  ...
  /**
   ...
   Defaults to `false`
   */
  var shouldScrollWhenTableViewIsEditing = false
  ...
  var additionalScrollToTopOffset: CGFloat = 0
  ...
  fileprivate(set) var gestureRecognizer: UIPanGestureRecognizer?
  ...
  var isTopViewControllerExtendedUnderNavigationBar: Bool {
   ...
   */
  func followScrollView(_ scrollableView: UIView, delay: Double = 0, scrollSpeedFactor: Double = 1, collapseDirection: NavigationBarCollapseDirection = .scrollDown, additionalOffset: CGFloat = 0, followers: [NavigationBarFollower] = []) {
   ...
   */
  func showNavbar(animated: Bool = true, duration: TimeInterval = 0.1, scrollToTop: Bool = false, completion showNavCompletion: (() -> Void)? = nil) {
    guard let _ = self.scrollableView, let visibleViewController = self.visibleViewController else { return }
   ...
   */
  func stopFollowingScrollView(showingNavbar: Bool = true) {
    if showingNavbar {
